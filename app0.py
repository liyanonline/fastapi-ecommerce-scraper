...
# # Check if Playwright binaries are installed, install if missing
...
# Core functions (unchanged from previous examples, abbreviated here for brevity)
...
def scrape_website(target_url: str, headers: dict = None, pages: int = 1, sleep_time: int = 1):
    ...
    for page in range(1, pages + 1):
        ...
        if 'amazon' in url:
            ...
            if html:
                all_data.extend(parse_html(html, target_url))
...
def run_scraper(product_name, currency_code, remove_symbol, pages):
    ...
    return f"Done! Files: {csv_file}, {graph_file}", csv_file, graph_file
